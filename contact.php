        ...
        <div class="lg:col-span-2 bg-gray-900 p-6 rounded-lg border border-gray-800">
            ...
            <form method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" required class="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded text-white">
                </div>
                <div>
                    <label class="block text-gray-300 mb-2">Your Email:</label>
                    <input type="email" name="email" required class="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded text-white">
                </div>
                <div>
                    <label class="block text-gray-300 mb-2">Subject:</label>
                    <input type="text" name="subject" required class="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded text-white">
                </div>
                <div>
                    <label class="block text-gray-300 mb-2">Message:</label>
                    <textarea name="message" rows="5" required class="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded text-white"></textarea>
                </div>
                <button type="submit" class="w-full bg-yellow-500 text-black px-6 py-3 rounded font-bold hover:bg-yellow-600">
                    Send Message
                </button>
            </form>
        </div>

        <!-- contact i + opening hours -->
            ...
            <div class="bg-gray-900 p-6 rounded-lg border border-gray-800 flex-1">
                ...
                <div class="space-y-4 text-gray-300">
                    <div>
                        ...
                        <a href="mailto:<?= htmlspecialchars($info['contact_email']) ?>" class="hover:text-yellow-500">
                            <?= htmlspecialchars($info['contact_email']) ?>
                        </a>
                    </div>
                    <div>
                        <span class="text-yellow-500 font-bold">Phone:</span><br>
                        <?= htmlspecialchars($info['contact_number']) ?>
                    </div>
                    <div>
                        <span class="text-yellow-500 font-bold">Address:</span><br>
                        <?= nl2br(htmlspecialchars($info['address'] ?? 'No address available.')) ?>
                    </div>
                </div>
            </div>
            <div class="bg-gray-900 p-6 rounded-lg border border-gray-800 flex-1 mt-0 border-t-0">
